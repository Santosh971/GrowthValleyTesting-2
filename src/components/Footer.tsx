...
export default function Footer() {
  ...
  const showLogo = mounted && hasLogo && footerLogo;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-4">
            <Link href="/" className="inline-block mb-6">
              {/* FIX: Both elements always rendered; CSS controls visibility to avoid hydration mismatch */}
              <>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={footerLogo || ''}
                  alt={siteName}
                  className={`h-16 w-auto ${showLogo ? 'block' : 'hidden'}`}
                />
                <span
                  className={`text-4xl font-semibold tracking-tight text-white dark:text-white ${showLogo ? 'hidden' : 'block'}`}
                >
                  {siteName.split(' ')[0]}
                  <span className="text-accent">{siteName.split(' ')[1] || ''}</span>
                </span>
        ...
        {/* Bottom Bar */}
        <div className="mt-12 pt-8 border-t border-brand-grey-800 dark:border-brand-grey-800">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-body-sm text-brand-grey-500 dark:text-brand-grey-500">
              {copyrightText}
            </p>
